...
def flow(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    flow = OAuthFlow(instance_url='https://test.salesforce.com', port=5050, salesfunk_path=tmp_path)
    return flow
...
def test_run_mocked(flow: OAuthFlow):
    ...
    with patch.object(flow, '_run') as mock_run, \
...
def test_run_oauth_flow_browser_open_returns_false(flow: OAuthFlow, capsys):
    flow._oauth_token = {"access_token": "mock", "instance_url": "https://test.salesforce.com"}
    flow._shutdown_trigger.wait = lambda: None
    
    with patch('webbrowser.open', return_value=False), \
        patch.object(flow._app, 'run'):
        flow._run()
